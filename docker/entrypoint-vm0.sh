...
#
# In protopulse mode, ebpf-agent is not needed (service-a reads /proc/net/tcp
# directly), so we skip it to reduce resource usage and avoid eBPF load errors
# on kernels without BTF/tracepoint support.
set -e

if [ "${LB_MODE}" = "protopulse" ]; then
    echo "LB_MODE=protopulse — skipping ebpf-agent (not needed)"
else
